    ...
    for (y, row) in map.enumerated() {
        for (x, col) in row.enumerated() {
            if col == MapLegend.asteroid.rawValue {
                asteroids.insert(Asteroid(x: Double(x), y: Double(y)))
            }
        }
    }
...
//let c = determineLineOfSight(for: Asteroid(x: 11, y: 13), on: map, asteroids: asteroidLocations)

var z = Asteroid(x: 3, y: 4)
var as1 = Asteroid(x: 4, y: 2)

var adfa = asteroidLocations
func collision(center: Asteroid, b: Asteroid, c: Asteroid) -> Bool {
//    for asteroid in adfa {
        let dy = center.y - b.y
        let dz = center.x - b.x
...
collision(center: Asteroid(x: 4, y: 2), b: Asteroid(x: 2, y: 2), c: Asteroid(x: 3, y: 2))
