...
var inputs = try String(contentsOf: inputFileURL!).split(separator: ",").compactMap { Int($0.trimmingCharacters(in: .whitespacesAndNewlines)) }
...
@discardableResult func Intcode(for codesPtr: inout [Int], with input: Int = 0, prevOutput: Int = 0, index: Int = 0, flag: Bool = false, base: Int = 0) -> (Int, Int) {
    var lastOutput = 0
    var codes = codesPtr
    var i = index
    var base = base
...
func getParameterMode(with number: Int) -> (ParameterMode?, ParameterMode?, ParameterMode?, Opcode?) {
    ...
    print(parameter1Mode, parameter2Mode, parameter3Mode)
